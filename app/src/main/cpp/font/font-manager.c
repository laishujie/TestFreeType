                                ...
                                const float size )
{
    ...
    LOGCATE("Unable to load \"%s\" (size=%.1f)\n", filename, size )
    return 0;
}
                                   ...
                                   const int italic )
{
    ...
    {
#if defined(_WIN32) || defined(_WIN64)
        LOGCATE("\"font_manager_get_from_description\" not implemented yet.\n" );
        return 0;
#endif
        filename = font_manager_match_description( self, family, size, bold, italic );
        if( !filename )
        {
            LOGCATE("No \"%s (size=%.1f, bold=%d, italic=%d)\" font available.\n",
...
#else
#  if defined _WIN32 || defined _WIN64
      LOGCATE("\"font_manager_match_description\" not implemented for windows.\n" );
      return 0;
#  endif
    char *filename = 0;
    ...
    if ( !match )
    {
        LOGCATE("fontconfig error: could not match family '%s'", family );
        return 0;
    }
    else
    {
